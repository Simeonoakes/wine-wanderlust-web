...
const links: { label: string; href?: string; action?: () => void }[] = [
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-md border-b border-border">
        <div className="max-w-7xl mx-auto px-8 flex items-center justify-between h-20">
          <Link to="/" className="flex items-center gap-3 whitespace-nowrap">
            <img src={logoAsset} alt="" className="h-24 w-24 object-contain" />
            ...
            {links.map((link) =>
              link.href ? (
                ...
                <button
                  ...
                  className="text-xs uppercase tracking-[0.1em] text-primary whitespace-nowrap hover:text-primary/80 transition-colors"
                >
                  {link.label}
                </button>
              )
                ...
                {links.map((link) =>
                  link.href ? (
                    ...
                    <button
                      ...
                      className="text-sm uppercase tracking-[0.1em] text-primary text-left"
                    >
                      {link.label}
                    </button>
                  )
